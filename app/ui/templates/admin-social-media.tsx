...
interface AdminSocialMediaTemplateProps {
  stats: SocialMediaStat[];
  platforms: SocialPlatform[];
}

export function AdminSocialMediaTemplate({ stats, platforms }: AdminSocialMediaTemplateProps) {
  ...
  const handleUpload = async () => {
    if (!title || !content || !file) {
      alert("Judul, Konten, dan Gambar harus diisi!");
      return;
    }
  ...
  return (
    <DashboardHeader headerTitle="Manajemen Publikasi">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm space-y-6">
            ...
            <div className="space-y-4">
              <div>
                <Txt variant="small" weight="bold" className="mb-2 block text-gray-400 uppercase tracking-widest">Judul Artikel</Txt>
                <input
                  type="text"
                  value={title}
                  onChange={(e) => setTitle(e.target.value)}
                  className="w-full p-4 mb-4 rounded-[16px] border-none bg-gray-50 focus:ring-2 focus:ring-red-primary/10 transition-all text-sm"
                  placeholder="Masukkan judul..."
                />
                <Txt variant="small" weight="bold" className="mb-2 block text-gray-400 uppercase tracking-widest">Konten / Caption</Txt>
                <textarea
                  value={content}
                  onChange={(e) => setContent(e.target.value)}
                  className="w-full h-32 p-6 rounded-[32px] border-none bg-gray-50 focus:ring-2 focus:ring-red-primary/10 transition-all resize-none text-sm leading-relaxed"
                  placeholder="Ceritakan kegiatan yayasan hari ini..."
                />
              </div>
              <div className="border-2 border-dashed border-gray-100 rounded-[32px] p-10 flex flex-col items-center justify-center gap-3 hover:bg-gray-50/50 transition-all group relative">
                ...
                <Txt variant="body" weight="medium" className="text-gray-400 group-hover:text-gray-900">
          ...
          <div className="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm space-y-6 flex flex-col">
            ...
            <div className="flex-grow aspect-video bg-gray-50 rounded-[32px] flex flex-col items-center justify-center border border-gray-50 overflow-hidden relative group">
              ...
              {!file && <Txt variant="small" className="text-gray-400 font-medium italic relative z-10">Belum ada media yang diunggah</Txt>}
            </div>
            <div className="flex gap-4">
              ...
              <Btn 
                variant="red" 
                shape="rounded" 
                className="flex-1 py-4 gap-2 shadow-lg shadow-red-primary/20"

...
import { ChevronLeft, Share2, Heart, ShieldCheck, Users, Calendar } from 'lucide-react';
import Link from 'next/link';
import { useParams } from 'next/navigation';
import { DataVerification } from '@/app/ui/molecules/DataVerification';

const donationSchema = z.object({
  amount: z.number({ message: "Nominal harus angka" }).min(10000, "Minimal donasi Rp 10.000"),
...
export default function DonationDetailPage() {
  ...
  const program = {
    ...
    category: "Pendidikan",
    description: "Bantu biaya sekolah untuk 50 anak yatim di Tasikmalaya agar mereka tetap bisa mengejar cita-cita meskipun dalam keterbatasan ekonomi.",
    ...
    <div className="bg-gray-50 min-h-screen pb-20">
      ...
      <div className="relative h-[400px] md:h-[500px] w-full overflow-hidden">
        ...
        <div className="absolute top-8 left-6 right-6 flex justify-between items-center">
          <Link href="/home/donasi" className="p-3 bg-white/20 backdrop-blur-md rounded-2xl text-white hover:bg-white/30 transition-all">
            <ChevronLeft size={24} />
          </Link>
        ...
        <div className="absolute bottom-12 left-8 right-8 text-white space-y-4">
           <Badge variant="solid" color="primary" className="bg-red-primary border-none">{program.category}</Badge>
           <Txt variant="h2" weight="bold" className="text-3xl md:text-5xl leading-tight max-w-3xl">{program.title}</Txt>
           <div className="flex items-center gap-6 text-white/80">
              <div className="flex items-center gap-2">
                 <Users size={18} />
                 <Txt variant="small" weight="bold">{program.donors} Donatur</Txt>
              </div>
              <div className="flex items-center gap-2">
                 <Calendar size={18} />
                 <Txt variant="small" weight="bold">{program.daysLeft} Hari Lagi</Txt>
              </div>
           </div>
        </div>
      </div>
      ...
      <Container className="max-w-6xl mx-auto -mt-12 relative z-10 px-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
             <Container variant="white" radius="2xl" className="p-8 shadow-xl shadow-black/5 border border-gray-100">
                <div className="space-y-6">
                   <div className="flex justify-between items-end">
                      <div className="space-y-1">
                         <Txt variant="caption" className="text-gray-400 font-bold uppercase tracking-widest">Terkumpul</Txt>
                         <Txt variant="h3" weight="bold" className="text-red-primary">Rp {program.raised.toLocaleString('id-ID')}</Txt>
                      </div>
                      <Txt variant="body" weight="bold" className="text-gray-400">Target: Rp {program.target.toLocaleString('id-ID')}</Txt>
                   </div>
                   
                   <div className="h-4 bg-gray-100 rounded-full overflow-hidden">
                      <div className="h-full bg-red-primary rounded-full relative" style={{ width: `${progress}%` }}>
                         <div className="absolute inset-0 bg-gradient-to-r from-white/20 to-transparent" />
                      </div>
                   </div>

                   <div className="pt-6 border-t border-gray-50">
                      <Txt weight="bold" className="text-xl mb-4 block">Tentang Program</Txt>
          ...
          <div className="space-y-6">
             <Container variant="white" radius="2xl" className="p-8 shadow-xl shadow-black/5 border border-gray-100 sticky top-8">
                <Txt variant="h4" weight="bold" className="mb-6 block">Donasi Sekarang</Txt>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                   ...
                   <div className="space-y-4 pt-4">
                      ...
                      <div className="flex items-center gap-2 px-1">
                         ...
                         />
                         <label htmlFor="anon" className="text-xs text-gray-500 font-medium cursor-pointer">Sembunyikan nama saya (Anonim)</label>
                      </div>
                   </div>
                   ...
                   <Txt variant="caption" className="text-gray-400 text-center block pt-2 italic">
